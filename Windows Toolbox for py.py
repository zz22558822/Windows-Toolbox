...
def show_admin_prompt():
    ...
    msg_box.setWindowTitle("Windows Toolbox")
    msg_box.setText("程式需要管理員權限。是否使用管理員啟動?")
    msg_box.setStandardButtons(QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
    return msg_box.exec()

# 若選擇 Yes 則使用管理員開啟
if not is_admin():
    user_choice = show_admin_prompt()
...
class Ui_Form(object):
    ...
    def hide_top_button(self): # 隱藏上方資料夾按鈕
        try:
            ...
            self.show_notification("成功", "隱藏上方資料夾，已完成。")
        except Exception as e:
    ...
    def show_top_button(self): # 顯示上方資料夾按鈕
        try:
            ...
            subprocess.run('taskkill /f /im explorer.exe', shell=True)
            subprocess.run('start explorer.exe', shell=True)
            self.show_notification("成功", "顯示上方資料夾，已完成。")
        except Exception as e:
    ...
    def hide_left_button(self): # 隱藏左側本機按鈕
        try:
            ...
            subprocess.run('taskkill /f /im explorer.exe', shell=True)
            subprocess.run('start explorer.exe', shell=True)
            self.show_notification("成功", "隱藏左側本機，已完成。")
        except Exception as e:
            QMessageBox.critical(None, "錯誤", "取消隱藏時出現錯誤，請重新再試或改用指令。")
    ...
    def show_left_button(self): # 顯示左側本機按鈕
        try:
            ...
            subprocess.run('taskkill /f /im explorer.exe', shell=True)
            subprocess.run('start explorer.exe', shell=True)
            self.show_notification("成功", "顯示左側本機，已完成。")
        except Exception as e:
            QMessageBox.critical(None, "錯誤", "取消隱藏時出現錯誤，請重新再試或改用指令。")
    ...
    def hide_OneDrive(self): # 隱藏 OneDrive
        try:
            ...
            subprocess.run('taskkill /f /im explorer.exe', shell=True)
            subprocess.run('start explorer.exe', shell=True)
            self.show_notification("成功", "隱藏 OneDrive 完成。")
        except Exception as e:
    ...
    def hide_Dropbox(self): # 隱藏 Dropbox
        try:
            ...
            subprocess.run('taskkill /f /im explorer.exe', shell=True)
            subprocess.run('start explorer.exe', shell=True)
            self.show_notification("成功", "隱藏 Dropbox 完成。")
        except Exception as e:
            QMessageBox.critical(None, "錯誤", "隱藏時出現錯誤，請重新再試。")
    ...
    def hide_Quick_access(self): # 隱藏快速存取
        try:
            ...
            subprocess.run('taskkill /f /im explorer.exe', shell=True)
            subprocess.run('start explorer.exe', shell=True)
            self.show_notification("成功", "隱藏 快速存取 完成。")
        except Exception as e:
            QMessageBox.critical(None, "錯誤", "隱藏時出現錯誤，請重新再試。")
    ...
    def show_Quick_access(self): # 顯示快速存取
        try:
            ...
            subprocess.run('taskkill /f /im explorer.exe', shell=True)
            subprocess.run('start explorer.exe', shell=True)
            self.show_notification("成功", "顯示 快速存取 完成。")
        except Exception as e:
    ...
    def hide_Internet(self): # 隱藏網路
        try:
            ...
            subprocess.run('taskkill /f /im explorer.exe', shell=True)
            subprocess.run('start explorer.exe', shell=True)
            self.show_notification("成功", "隱藏網路 完成。")
        except Exception as e:
            QMessageBox.critical(None, "錯誤", "隱藏時出現錯誤，請重新再試。")
    ...
    def show_Internet(self): # 顯示網路
        try:
            ...
            subprocess.run('taskkill /f /im explorer.exe', shell=True)
            subprocess.run('start explorer.exe', shell=True)
            self.show_notification("成功", "顯示網路 完成。")
        except Exception as e:
            QMessageBox.critical(None, "錯誤", "顯示時出現錯誤，請重新再試。")
    ...
    # 運行系統修復 Py無法直接調用管理員權限 因此另存BAT後執行
...
'''
            ...
            self.show_notification("成功", "已開始運行系統修復 SFC。")
